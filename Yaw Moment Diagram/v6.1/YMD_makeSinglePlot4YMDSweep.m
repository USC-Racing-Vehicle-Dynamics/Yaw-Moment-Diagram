...
function YMD_makeSinglePlot4YMDSweep(param, sweptParam, sweep, sweptParamIndex)
...
AxSweepData.full = zeros(length(SA.range), length(Delta.range), length(SX.range));
AySweepData.full = zeros(length(SA.range), length(Delta.range));
MSweepData.full = zeros(length(SA.range), length(Delta.range));
...
for z = 1: length(SX.range)
    ...
    for x = 1: length(SA.range)
        ...
        for y = 1: length(Delta.range)
            ...
            AxSweepData.full(x, y, z) = Ax;
            AySweepData.full(x, y, z) = Ay;       
            MSweepData.full(x, y, z) = (FY_lf*cos(delta_lf) + FY_rf*cos(delta_rf))*a + ...
...
for y = 1: length(Delta.range)
    ...
    for z = 1: length(SX.range)
        ...
        Ay_SALine = squeeze(AySweepData.full(1: length(SA.range), y, z));
        M_SALine = squeeze(MSweepData.full(1: length(SA.range), y, z));
        Ax_SALine = squeeze(AxSweepData.full(1: length(SA.range), y, z));
...
for x = 1: length(SA.range)
    ...
    for z = 1: length(SX.range)
        ...
        Ay_deltaLine = squeeze(AySweepData.full(x, 1: length(Delta.range), z));
        M_deltaLine = squeeze(MSweepData.full(x, 1: length(Delta.range), z));
        Ax_deltaLine = squeeze(AxSweepData.full(x, 1: length(Delta.range), z));
...
for x = 1: length(SA.range)
    ...
    for y = 1: length(Delta.range)
        ...
        Ay_SXLine = squeeze(AySweepData.full(x, y, 1: length(SX.range)));
        M_SXLine = squeeze(MSweepData.full(x, y, 1: length(SX.range)));
        Ax_SXLine = squeeze(AxSweepData.full(x, y, 1: length(SX.range)));
